...
var addworklist = $('#addworklist');
var addprojectlist = $("#addprojectlist");
var addedulist = $("#addedulist");
var addlanlist = $("#addlanlist");

$(document).ready(function(){
	console.log(<%=editResumeId%>);
	$('#addworklist').datagrid({
		onClickRow: function(index,row){
			console.log("work");
			//将datagrid中选中的行内容填充到表单
            $('#add_form_resume_work').form('load',{
	...
	$('#addprojectlist').datagrid({
		onClickRow: function(index,row){
			  console.log("project");
			  $('#add_form_resume_project').form('load',{
...
function saveOrUpdateResumeWork(){
	var resumeId = $("#add_input_resume_id").val();//获取创建的简历id
	if(resumeId!=null&&resumeId!=''){
		$("#addwork_resumeid").val(resumeId);//将获取到的简历id设置到add_form_resume_work表单中
		$('#add_form_resume_work').form('submit', {
			url : '${pageContext.request.contextPath}/resumeAction!saveOrUpdateResumeWork.action',
			success : function(d) {
				var json = $.parseJSON(d);
				if (json.success) {
					addworklist.datagrid('load');
					//ad.dialog('close');
					$.messager.show({
		                title:'操作提示',
		                msg:json.message,
...
function saveOrUpadateResumeProject(){
	var resumeId = $("#add_input_resume_id").val();//获取创建的简历id
	if(resumeId!=null&&resumeId!=''){
		$("#addproject_resumeid").val(resumeId);//将获取到的简历id设置到add_form_resume_project表单中
		$('#add_form_resume_project').form('submit', {
			url : '${pageContext.request.contextPath}/resumeAction!saveOrUpdateResumeProject.action',
			success : function(d) {
				var json = $.parseJSON(d);
				if (json.success) {
					addprojectlist.datagrid('load');
					$.messager.show({
		                title:'操作提示',
		                msg:json.message,
		                timeout:2000,
...
function saveOrUpdateResumeLan(){
	var resumeId = $("#add_input_resume_id").val();//获取创建的简历id
	if(resumeId!=null&&resumeId!=''){
		$("#addlan_resumeid").val(resumeId);//将获取到的简历id设置到add_form_resume_lan表单中
		$('#add_form_resume_lan').form('submit', {
			url : '${pageContext.request.contextPath}/resumeAction!saveOrUpdateResumeLan.action',
			success : function(d) {
				var json = $.parseJSON(d);
				if (json.success) {
					addlanlist.datagrid('load');
					//ad.dialog('close');
					$.messager.show({
		                title:'操作提示',
		                msg:json.message,
...
function saveOrUpdateResumeEdu(){
	var resumeId = $("#add_input_resume_id").val();//获取创建的简历id
	if(resumeId!=null&&resumeId!=''){
		$("#addedu_resumeid").val(resumeId);//将获取到的简历id设置到add_form_resume_edu表单中
		$('#add_form_resume_edu').form('submit', {
			url : '${pageContext.request.contextPath}/resumeAction!saveOrUpdateResumeEdu.action',
			success : function(d) {
				var json = $.parseJSON(d);
				if (json.success) {
					addedulist.datagrid('load');
					//ad.dialog('close');
					$.messager.show({
		                title:'操作提示',
		                msg:json.message,
...
function saveOrUpdateResumeJobInten(){
	var resumeId = $("#add_input_resume_id").val();//获取创建的简历id
	if(resumeId!=null&&resumeId!=''){
		$("#addjobinten_resumeid").val(resumeId);//将获取到的简历id设置到add_form_resume_jobintension表单中
		$('#add_form_resume_jobintension').form('submit', {
...
function saveOrUpdateResumeOtherInfo(){
	var resumeId = $("#add_input_resume_id").val();//获取创建的简历id
	if(resumeId!=null&&resumeId!=''){
		$("#addotherinfo_resumeid").val(resumeId);//将获取到的简历id设置到add_form_resume_otherinfo表单中
		$('#add_form_resume_otherinfo').form('submit', {
...
var workToolbar = [{
    ...
    iconCls:'icon-remove',
    handler:function(){
    	var rows = addworklist.datagrid('getChecked');
		var ids = [];
		if (rows.length > 0) {
			$.messager
					.confirm(
							...
							function(r) {
								if (r) {
									...
									$.ajax({
												...
												success : function(d) {
													addworklist.datagrid('load');
													addworklist.datagrid('unselectAll');
													$.messager.show({
														title : '提示',
														msg : d.msg
													});
...
var projectToolbar = [{
    ...
    iconCls:'icon-remove',
    handler:function(){
    	var rows = addprojectlist.datagrid('getChecked');
		var ids = [];
		if (rows.length > 0) {
			$.messager
					.confirm(
							...
							function(r) {
								if (r) {
									...
									$.ajax({
												...
												success : function(d) {
													addprojectlist.datagrid('load');
													addprojectlist.datagrid('unselectAll');
													$.messager.show({
														title : '提示',
														msg : d.msg
													});
...
var eduToolbar = [{
    ...
    iconCls:'icon-remove',
    handler:function(){
    	var rows = addedulist.datagrid('getChecked');
		var ids = [];
		if (rows.length > 0) {
			$.messager
					.confirm(
							...
							function(r) {
								if (r) {
									...
									$.ajax({
												...
												success : function(d) {
													addedulist.datagrid('load');
													addedulist.datagrid('unselectAll');
													$.messager.show({
														title : '提示',
														msg : d.msg
													});
...
var lanToolbar = [{
    ...
    iconCls:'icon-remove',
    handler:function(){
    	var rows = addlanlist.datagrid('getChecked');
		var ids = [];
		if (rows.length > 0) {
			$.messager
					.confirm(
							...
							function(r) {
								if (r) {
									...
									$.ajax({
												...
												success : function(d) {
													addlanlist.datagrid('load');
													addlanlist.datagrid('unselectAll');
													$.messager.show({
														title : '提示',
														msg : d.msg
													});
...
function deleteOtherInfo(){
	var otherInfoId = $("#addotherinfolist").val();
...
function deleteJobInten(){
	var jobIntenId = $("#addjobintenlist").val();
...
<div id="addResumeInfoTab" class="easyui-tabs" style="width:100%;height:480px;">
		<div title="个人信息" data-options="iconCls:'icon-help'" style="padding:10px">
			 <div align="center" style="padding:20px;">
                <form id="add_form_resume_create" method="post">
					<table>
					<tr><td><input id="add_input_resume_id" name="resume.id" hidden="true" /></td></tr>
					<tr>
						<th>简历编号</th>
		...
		<div title="求职意向" data-options="iconCls:'icon-help'" style="padding:10px">
			<div align="center" style="padding:20px;">
                	<form id="add_form_resume_jobintension" method="post">
					<table>
					<tr>
					<td><input hidden="true" id="addjobintenlist" name="resumeJobInten.jobIntenId"></input></td>
					<td><input hidden="true" id="addjobinten_resumeid" name="resumeJobInten.resumeId" ></input></td>
					</tr>
					<tr>
		...
		<div title="工作经历" data-options="iconCls:'icon-help'">
			<div align="center">
			      <!-- 已经添加的工作经验 -->
					<table class="easyui-datagrid" id="addworklist" style="width:99%;height:200px"
                	...
                	<form id="add_form_resume_work" method="post">
					<table>
					<tr>
					<td><input hidden="true" name="resumeWork.workId"></input></td>
					<td><input hidden="true" id="addwork_resumeid" name="resumeWork.resumeId"></input></td>
					</tr>
					<tr>
					...
					<tr>
					 ...
					 <th></th>
					 <td colspan=4 align="center" style="width:100px;height:30px;">
					 <a href="#" class="easyui-linkbutton" onclick="cleanFormFun('add_form_resume_work')" data-options="iconCls:'icon-cancel'">清空</a>
					 </td>
					 </tr>
				</table>
			</form>		
		...
		<div title="项目经验" data-options="iconCls:'icon-help'" >
			<div align="center">
			  <table class="easyui-datagrid"  id="addprojectlist"  style="width:99%;height:200px"
		        ...
		        <thead>
                 ...
                 <form id="add_form_resume_project" method="post">
					<table>
					<tr>
					<td><input hidden="true" name="resumeProject.projectId"></input></td>
					<td><input hidden="true" id="addproject_resumeid" name="resumeProject.resumeId"></input></td>
					</tr>
					 <tr>
					...
					<tr>
					 ...
					 <td colspan=4 align="center" style="width:100px;height:30px;">
					 <a href="#" class="easyui-linkbutton" onclick="cleanFormFun('add_form_resume_project')" data-options="iconCls:'icon-cancel'">清空</a>
					 </td>
					 </tr>
				</table>
			</form>		
		...
		<div title="教育经历" data-options="iconCls:'icon-help'" >
					<div align="center" >
					     <table class="easyui-datagrid"  id="addedulist"  style="width:99%;height:200px"
				        ...
				        <thead>
		                	...
		                	<form id="add_form_resume_edu" method="post">
							<table>
							<tr>
							<td><input hidden="true" name="resumeEdu.eduId"></input></td>
							<td><input hidden="true" id="addedu_resumeid" name="resumeEdu.resumeId"></input></td>
							</tr>
							<tr>
								<th>开始日期<span style="color:red">*</span></th>
							...
							<tr>
							 ...
							 <td colspan=4 align="center" style="width:100px;height:30px;">
							 <a href="#" class="easyui-linkbutton" onclick="cleanFormFun('add_form_resume_edu')" data-options="iconCls:'icon-cancel'">清空</a>
							 </td>
							 </tr>
						</table>
					</form>		
		...
		<div title="语言水平" data-options="iconCls:'icon-help'" >
					...
					<div align="center" >
						...
						<table class="easyui-datagrid"  id="addlanlist"  style="width:99%;height:200px"
				        ...
				        <thead>
		                	...
		                	<form id="add_form_resume_lan" method="post">
							<table>
							<tr>
							<td><input hidden="true" name="resumeLan.lanId" /></td>
							<td><input hidden="true" id="addlan_resumeid" name="resumeLan.resumeId" /></td>
							</tr>
							<tr>
							...
							<tr>
							 ...
							 <td  colspan=4 align="center" style="width:100px;height:30px;">
							 <a href="#" class="easyui-linkbutton" onclick="cleanFormFun('add_form_resume_lan')" data-options="iconCls:'icon-cancel'">清空</a>
							 </td>
							 </tr>
						</table>
					</form>		
		...
		<div title="其他信息" data-options="iconCls:'icon-help'" style="padding:10px">
				<div align="center" style="padding:20px;">
	                <form id="add_form_resume_otherinfo" method="post">
					<table>
					<tr>
						<td><input hidden="true" id="addotherinfolist" name="resumeOtherInfo.otherId"></input></td>
						<td><input hidden="true" id="addotherinfo_resumeid" name="resumeOtherInfo.resumeId"></input></td>
					</tr>
					<tr>
